...
输入全文
  ...
  → 拼回全文复检
  → 仍 >=20：归因定向 LLM；第2轮仅在当前最佳分数为 `[20%, 30%)` 时执行，只有全文分数下降才接受
```
...
- [x] **P0 - 高 AI 率自动定向二次改写**：改写结果不达标时，按 V2 决策路径归因选择高贡献块，默认最多两轮；第2轮仅在当前最佳分数低于30%时执行，每轮全文复检，只接受风险下降的候选，并记录单轮耗时。
...
- **20260909 · V2 检测、风险路由与 AI Detector**
  ...
  - 首轮未达标时支持随机森林路径归因和最多两轮定向 LLM；第2轮仅处理 `[20%, 30%)` 的接近达标结果，输出校验或全文升分时回滚
